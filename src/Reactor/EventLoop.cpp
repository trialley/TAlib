...
EventLoop::EventLoop()
	: m_looping(false),
	  m_threadId(CurrentThread::tid()),	 //创建这个loop的线程就绑定了这个loop
	...
	if (t_loopInThisThread) {  //每个线程只有一个EventLoop对象 , 如果当前线程创建了其他 EventLoop对象,则终止程序.
...
void EventLoop::queueInLoop(const Functor& cb) {
	LOG_TRACE << "EventLoop::queueInLoop()";
	{
		...
		m_pendingFunctors.push_back(std::move(cb));	 //将该函数添加到自己的待处理函数中
	}

	if (!isInloopThread() || m_callingPendingFunctors) {  //然后唤醒
		wakeup();
	}
}
